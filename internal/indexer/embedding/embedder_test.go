...
func TestPrepareEntries(t *testing.T) {
	entries := []embedding.EntryText{
		{UUID: "e1", SessionID: "s1", Source: "claude", Text: "short text for embedding"},
	}
	requests, mapping := embedding.PrepareEntries(entries, 2000, 200)
	if len(requests) != 1 {
		t.Fatalf("expected 1 request, got %d", len(requests))
	}
	if requests[0].ID != "claude:s1:e1_0" {
		t.Fatalf("expected id=claude:s1:e1_0, got %s", requests[0].ID)
	}
	if len(mapping) != 1 {
