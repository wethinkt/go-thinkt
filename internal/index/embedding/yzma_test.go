...
func TestEmbedder(t *testing.T) {
	modelPath, err := llm.ModelPath(DefaultModelID)
	if err != nil {
...
func TestDefaultModelPath(t *testing.T) {
	p, err := llm.ModelPath(DefaultModelID)
	if err != nil {
		t.Fatalf("ModelPath: %v", err)
	}
	if p == "" {
		t.Fatal("ModelPath returned empty string")
	}
	// Should end with the expected model filename.
	spec, _ := llm.LookupModel(DefaultModelID)
