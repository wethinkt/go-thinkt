...
func init() {
	...
	sharePushCmd.Flags().StringVar(&sharePushTitle, "title", "", "session title")
	sharePushCmd.Flags().StringVar(&sharePushTags, "tags", "", "comma-separated tags")
	sharePushCmd.Flags().StringVarP(&sharePushProject, "project", "p", "", "project path or ID")
	sharePushCmd.Flags().StringVarP(&sharePushSession, "session", "s", "", "session ID or path")
	sharePushCmd.Flags().StringSliceVar(&sharePushSources, "source", nil, "filter by source")
	sharePushCmd.Flags().Bool("no-thinking", false, "exclude thinking blocks")
	sharePushCmd.Flags().Bool("no-tools", false, "exclude tool use and results")
	sharePushCmd.Flags().Bool("no-media", false, "exclude images and documents")
	sharePushCmd.Flags().Bool("system", false, "include system entries")
